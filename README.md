...
1. Включить и выделить место <br>
Win+R → sysdm.cpl → вкладка «Защита системы» → выдели диск C: → «Настроить»: <br>

Включить защиту системы <br>
Ползунок «Максимальное использование» — поставь 5–10% (≈10–20 ГБ). Одна точка в среднем 0.5–2 ГБ, на 7+ хватит с запасом. Когда место кончится — Windows автоматически удалит самые старые. Это и есть встроенный механизм «оставлять только свежие». <br>
ОК → ОК. <br>
...
-StartWhenAvailable — если комп был выключен в 09:00, точка создастся при первом включении. <br>
RestorePointType MODIFY_SETTINGS — обычный тип, без ограничений. <br>
Время 09:00 подменяй на удобное. <br>
Проверка <br>
